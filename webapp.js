...
toggleWrap.innerHTML = `<button class="course-mode-btn ${this.mode === 'quick' ? 'active' : ''}" data-mode="quick">⚡ Quick Review</button> <button class="course-mode-btn ${this.mode === 'detailed' ? 'active' : ''}" data-mode="detailed">📖 Detailed</button>`;
...
return ` <div class="cases-card" onclick="Cases.openCase(${idx})"> <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:8px;"> <div class="cases-card-title">${c.title}</div> ${statusHtml} </div> <div class="cases-card-complaint">${c.chief_complaint}</div> <div style="display:flex; align-items:center; gap:8px; margin-top:8px;"> <span class="cases-diff-badge ${c.difficulty}">${diffLabel[c.difficulty] || c.difficulty}</span> <span style="font-size:11px; color:var(--text-hint);">Max: ${c.max_score} pts</span> </div> </div>`;
}).join(’’);
},
...
el.innerHTML = `<div class="ac-card-emoji">${card.emoji}</div> <div class="ac-card-info"> <div class="ac-card-title">${card.title}</div> <div class="ac-card-meta">${card.actions.length} actions · ${card.traps.length} exam traps</div> </div> <div style="color:var(--text-hint); font-size:18px;">›</div>`;
...
el.innerHTML = `<div class="hy-card-num">Ch.${card.chapter}</div> <div class="hy-card-info"> <div class="hy-card-title">${card.title.replace(/^Ch\.\d+\s*/, '')}</div> <div class="hy-card-meta">${card.bullets.length} key points</div> </div> <div style="color:var(--text-hint); font-size:18px;">›</div>`;
el.addEventListener(‘click’, () => { haptic(‘light’); this.openCard(idx); });
list.appendChild(el);
});
},
...
el.innerHTML = `<div class="rot-card-icon">${item.icon}</div> <div class="rot-card-info"> <div class="rot-card-title">${item.title}</div> <div class="rot-card-desc">${item.desc}</div> </div> <div style="color:var(--text-hint); font-size:18px;">›</div>`;
...
btn.innerHTML = `<div class="rot-card-icon">${set.icon}</div> <div class="rot-card-info"> <div class="rot-card-title">${set.title}</div> <div class="rot-card-desc">${set.pairs.length} pairs to match</div> </div> <div style="color:var(--text-hint); font-size:18px;">›</div>`;
...
card.innerHTML = `<div class="action-cat-icon">${cat.icon}</div> <div class="action-cat-title">${cat.title}</div> <div class="action-cat-count">${cat.count} guides</div>`;
...
card.innerHTML = `<div class="action-guide-icon">${g.icon}</div> <div class="action-guide-title">${g.title}</div> ${g.has_template ? '<div class="action-guide-badge">📋 Template</div>' : ''} <div style="color:var(--text-hint);">›</div>`;
